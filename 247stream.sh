...
# To use with a static in the background, remove the streamlink pipe part in the ffmpeg command, fill the ${BACKGROUND} variable, and replace the first "-i" line with this :
# Example:
...
do
  ...
  then
    ...
    streamlink -O http://ustream.tv/channel/iss-hdev-payload best | /usr/bin/ffmpeg -loglevel warning -re -threads "${THREAD_COUNT}" \
      -i - \
